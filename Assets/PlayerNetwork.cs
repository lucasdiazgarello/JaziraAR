...
public class PlayerNetwork : NetworkBehaviour
{
    ...
    void OnClientConnected(ulong clientId)
    {
        ...
        //TurnManager.Instance.UpdateButtonInteractivity();
    ...
    public void EndTurn()
    {
        if (IsServer)  // Asegúrate de que solo el servidor modifique el turno actual.
        {
            Debug.Log("is server de end turn");
            currentTurnIndex++;
    ...
    {
        Debug.Log("Entre a notificar el paso de turno");
        currentTurnIndex = newTurnIndex;

        Debug.Log("Ahora el turno lo tiene el índice: " + currentTurnIndex);

        // Suponiendo que tienes una referencia a tu TurnManager:
        Debug.Log("el ownerclientId para ver si es el turno es: " + NetworkObject.OwnerClientId);

        // Antes de llamar a IsMyTurn, puedes verificar qué valor tiene playerIDs[currentTurnIndex]
        Debug.Log("Valor actual de playerIDs en el índice actual: " + playerIDs[currentTurnIndex]);

        TurnManager.Instance.UpdateButtonState(IsMyTurn(NetworkObject.OwnerClientId));
    }

    public bool IsMyTurn(ulong clientId)
    {
        // Aquí también puedes agregar un log para ver el valor de clientId que se está pasando
        Debug.Log("El clientId pasado a IsMyTurn es: " + clientId);

        bool isTurn = (playerIDs[currentTurnIndex] == (int)clientId);

        // Puedes verificar también el resultado de la comparación
        Debug.Log("Resultado de la comparación en IsMyTurn: " + isTurn);
